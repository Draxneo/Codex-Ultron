...
The app is running locally, and the old Supabase data export exists. The old live Supabase project is still configured in `.env`, so staging must use separate credentials.
...
The importer refuses to run against the old live Supabase project.
...
## To Actually Create Staging

1. Create a new Supabase project named something like `UltraOffice2 Staging`.
...
6. Point the app at the staging URL and anon key.
