  ...
  THEN
    IF NOT EXISTS (
      ...
      INSERT INTO public.job_reminders (job_id, reminder_type, scheduled_for) VALUES
        (NEW.id, 'day_before', ((NEW.scheduled_date::date - 1)::text || ' 09:00 America/Chicago')::timestamptz),
        (NEW.id, 'morning_of', (NEW.scheduled_date::date::text || ' 07:00 America/Chicago')::timestamptz);
    END IF;
  END IF;
  RETURN NEW;
END;
