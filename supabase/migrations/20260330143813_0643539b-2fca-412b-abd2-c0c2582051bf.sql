...
AS $function$
  ...
  enriched AS (
    SELECT
      ...
      -- Relevance score: prefix matches rank higher
      CASE
        WHEN sp.raw_search IS NULL THEN 0
        -- Exact first+last prefix match = best
    ...
    WHERE
      (sp.raw_search IS NULL OR
       -- Phone search: 4+ digits, match normalized phone
       ...
       (sp.raw_search IS NOT NULL AND (
         -- Single word: prefix match on name/company, fallback to substring on other fields
         ...
         OR
         -- Multi-word: first word matches first_name prefix, remaining words match last_name/company prefix
         (array_length(sp.words, 1) >= 2 AND (
           -- "Ron H" → first_name starts with "ron", last_name starts with "h"
           (lower(COALESCE(c.first_name, '')) LIKE sp.words[1] || '%'
            AND lower(COALESCE(c.last_name, '')) LIKE sp.words[2] || '%')
           OR
           -- Also try last_name first: "Haley R" → last_name starts with "haley", first_name starts with "r"
           (lower(COALESCE(c.last_name, '')) LIKE sp.words[1] || '%'
            AND lower(COALESCE(c.first_name, '')) LIKE sp.words[2] || '%')
           OR
           -- Company multi-word: each word is a prefix of a company word
           (lower(COALESCE(c.company, '')) LIKE '%' || sp.words[1] || '%'
            AND lower(COALESCE(c.company, '')) LIKE '%' || sp.words[2] || '%')
           OR
           -- Full name substring fallback for address/city searches
