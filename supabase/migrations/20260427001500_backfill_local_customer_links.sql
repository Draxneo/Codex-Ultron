-- Priority 2 cleanup: make HCP-era jobs/estimates visible from local customer detail.

CREATE INDEX IF NOT EXISTS idx_jobs_hcp_customer_id ON public.jobs(hcp_customer_id);
CREATE INDEX IF NOT EXISTS idx_estimates_hcp_customer_id ON public.estimates(hcp_customer_id);
CREATE INDEX IF NOT EXISTS idx_customers_hcp_customer_id ON public.customers(hcp_customer_id);

UPDATE public.estimates e
SET customer_id = c.id
FROM public.customers c
WHERE e.customer_id IS NULL
  AND e.hcp_customer_id IS NOT NULL
  AND c.hcp_customer_id = e.hcp_customer_id;

UPDATE public.jobs j
SET customer_id = c.id
FROM public.customers c
WHERE j.customer_id IS NULL
  AND j.hcp_customer_id IS NOT NULL
  AND c.hcp_customer_id = j.hcp_customer_id;
...
AS $$
  ...
  JOIN public.jobs j
    ...
    OR (
      ...
      AND j.hcp_customer_id IS NOT NULL
      AND c.hcp_customer_id IS NOT NULL
      AND j.hcp_customer_id = c.hcp_customer_id
    )
  GROUP BY c.id
$$;
