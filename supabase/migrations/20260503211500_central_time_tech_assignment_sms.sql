-- Keep tech assignment SMS times in San Antonio / Central time.
-- The job tables store appointment windows as timestamptz. Every text that
-- leaves this trigger must convert that stored value to America/Chicago.
...
BEGIN
  ...
  IF _supabase_url IS NULL THEN
    PERFORM public.log_system_error(
      'trigger',
      'notify_tech_on_assign_or_reschedule',
      'SUPABASE_URL missing from vault',
      'critical',
      NULL,
  ...
  -- After 5pm CT for tomorrow's jobs, resend the consolidated digest instead
  -- of sending a one-off text for every adjustment.
