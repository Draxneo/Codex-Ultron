...
export default function TechFormPublic() {
  ...
  const handleSubmit = async () => {
    ...
    if (!isDemo && job.id) {
      ...
      // Map submitted form sections to workflow step timestamp fields.
      // This closes the loop: form submission → workflow timestamp → machine advances.
      try {
        // 1. Get the step_groups present in THIS form's fields
        const submittedGroups = new Set(
          fields
            .filter(f => f.step_group)
            .map(f => f.step_group as string)
        );

        // 2. Load the workflow definition for this job type
        ...
        if (wfDef?.steps) {
          const steps = typeof wfDef.steps === "string"
            ? JSON.parse(wfDef.steps)
            : wfDef.steps;

          // 3. Find workflow steps whose form_sections overlap the submitted groups
          const patches: Record<string, string> = {};
          for (const step of steps) {
            if (!step.form_sections || !step.timestamp_field) continue;
            const hasOverlap = step.form_sections.some((s: string) =>
              submittedGroups.has(s)
            );
            if (hasOverlap) {
              // Check if timestamp already stamped — don't overwrite
              if (!(job as any)[step.timestamp_field]) {
                patches[step.timestamp_field] = new Date().toISOString();
              }
            }
          }

          // 4. Stamp matching timestamp fields on the job + log each to activity_log
          if (Object.keys(patches).length > 0) {
            await supabase.from("jobs").update(patches).eq("id", job.id);
            console.log("Workflow timestamps stamped:", Object.keys(patches));

            // Log each stamped workflow milestone
            const logEntries = Object.keys(patches).map(field => ({
              job_id: job.id,
              action: `workflow_stamp_${field.replace(/_at$/, "")}`,
              performed_by: employee?.name || "Tech",
              details: `Auto-stamped ${field} via tech form submission`,
            }));
            await supabase.from("activity_log").insert(logEntries);
          }
        }

        // 5. Trigger auto-advance-workflow to fire the autopilot chain
        supabase.functions.invoke("auto-advance-workflow", {
          body: { job_id: job.id, trigger_step: "form_submitted" },
        }).catch((e: any) => console.error("auto-advance failed:", e));
      } catch (e) {
        console.error("Workflow advancement error:", e);
        // Don't block form submission — just log it
      }
